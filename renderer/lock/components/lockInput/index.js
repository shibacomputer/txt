  ...
  return html`
    <section class=${style.lock}>
      <div class=${style.unlocker}>
        ...
        <div class="${style.inputfield} ${state.error? style.headShake : ''}">
          <input placeholder=${state.error? opts.error : opts.placeholder} autocomplete="current-password" type=${state.show? 'text' : 'password'} class="${style.input} ${style.ready} ${state.error? style.error : ''}" onkeyup=${update} onfocus=${update}/>
          <button class="${style.button} ${state.error? style.error : ''}" onclick=${verify}>${opts.verb}</button>
        </div>
      </div>
    </section>
  `
